...
static constexpr double DEFAULT_KP = 0.0005;
static constexpr double DEFAULT_KD = 0.0;
...
struct MotorControlTask {
  ...
  MotorControlTask() : motor_pid(DEFAULT_KP, DEFAULT_KI, DEFAULT_KD), out(90.0) { }
  ...
  void run() {
    ...
    double error = ctx.target_ticks_per_s - ctx.ticks_per_s;
    ctx.ticks_error = error;
    out += motor_pid.getCmdAutoStep(ctx.target_ticks_per_s, ctx.ticks_per_s);
